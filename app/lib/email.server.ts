...
function buildAlertHtml(settings: ShopSettings, audit: AuditRunWithFindings) {
  ...
  const rows = audit.findings.slice(0, 20).map((f) => `<tr><td>${escapeHtml(f.severity)}</td><td>${escapeHtml(f.productTitle)}</td><td>${escapeHtml(f.sku ?? "—")}</td><td style="text-align:right">${escapeHtml(formatMoney(f.priceAmount, currencyCode))}</td><td style="text-align:right">${escapeHtml(formatMoney(f.costAmount, currencyCode))}</td><td>${escapeHtml(getCostSourceLabel(f.costSource))}</td><td style="text-align:right">${escapeHtml(f.inventoryQuantity ?? "—")}</td><td style="text-align:right">${escapeHtml(formatMoney(f.inventoryRiskAmount, currencyCode))}</td><td style="text-align:right">${escapeHtml(f.marginBps == null ? "—" : `${(f.marginBps / 100).toFixed(1)}%`)}</td></tr>`).join("");
