...
function calculateInventoryRiskAmount(gapToTargetAmount, inventoryQuantity) { if (gapToTargetAmount == null || inventoryQuantity == null) return null; return roundMoney(Math.max(0, inventoryQuantity) * gapToTargetAmount); }
function auditVariant(input, minimumMarginBps) {
  if (input.costAmount == null) return { ...input, costAmount: null, profitAmount: null, marginBps: null, targetProfitAmount: null, gapToTargetAmount: null, inventoryRiskAmount: null, severity: "MISSING_COST" };
  ...
  if (marginBps < 0) return { ...input, profitAmount, marginBps, targetProfitAmount, gapToTargetAmount, inventoryRiskAmount, severity: "LOSS" };
  if (marginBps < minimumMarginBps) return { ...input, profitAmount, marginBps, targetProfitAmount, gapToTargetAmount, inventoryRiskAmount, severity: "LOW_MARGIN" };
  return null;
}
...
const low = auditVariant({ variantId: "2", productTitle: "B", priceAmount: 100, costAmount: 80, inventoryQuantity: 8 }, 3000);
assert.equal(low.severity, "LOW_MARGIN");
assert.equal(low.profitAmount, 20);
assert.equal(low.gapToTargetAmount, 10);
assert.equal(low.inventoryRiskAmount, 80);
const loss = auditVariant({ variantId: "3", productTitle: "C", priceAmount: 100, costAmount: 120, inventoryQuantity: 3 }, 3000);
assert.equal(loss.severity, "LOSS");
assert.equal(loss.profitAmount, -20);
assert.equal(loss.gapToTargetAmount, 50);
assert.equal(loss.inventoryRiskAmount, 150);
